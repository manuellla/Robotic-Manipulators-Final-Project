...
[theta_return, pos_return, theta_traj_return, err_return, u_return, err_pos_return, err_rpy_return, err_axang_return] = ...
...
u_traj = [u_P0, u_P1, u_rect, u_return];
err_pos = [err_pos_P0, err_pos_P1, err_pos_rect, err_pos_return];
err_rpy = [err_rpy_P0, err_rpy_P1, err_rpy_rect, err_rpy_return];
err_axang = [err_axang_P0, err_axang_P1, err_axang_rect, err_axang_return];
...
% Vetor de tempo para os gráficos
tempo = (0:size(theta_traj,2)-1) * dt;
...
while (norm(e - e_ant) > epsilon && i < max_iter)
    ...
    rpy_err = rotm2rpy(R_err);  % Já retorna [roll; pitch; yaw]
...
for i = 1:length(t)
    ...
    rpy_err = [rpy_err(3); rpy_err(2); rpy_err(1)];  % [roll; pitch; yaw]
...
function rpy = rotm2rpy(R)
    % Extrai roll, pitch, yaw da matriz de rotação
    roll = atan2(R(3,2), R(3,3));
    pitch = atan2(-R(3,1), sqrt(R(3,2)^2 + R(3,3)^2));
    yaw = atan2(R(2,1), R(1,1));
    
    rpy = [roll; pitch; yaw];  % Retorna [roll; pitch; yaw]
end
